...
def parse_varm(data: ParsingDataIn) -> dict[str, VDataFrame]:
    ...
    if not isinstance(data.varm, dict):
        raise TypeError("'varm' must be a dictionary of DataFrames.")
...
def parse_varp(data: ParsingDataIn) -> dict[str, VDataFrame]:
    if not len(data.varp):
        generalLogger.debug("    7. \u2717 'varp' was not given.")
        return {}
    
    if not isinstance(data.varp, dict):
        raise TypeError("'varp' must be a dictionary of 2D numpy arrays or pandas DataFrames.")
