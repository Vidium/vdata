...
def check_time_match(data: ParsingDataIn) -> None:
    ...
    if data.timepoints.empty and data.time_list is not None:
        data.timepoints["value"] = np.unique(data.time_list, equal_nan=False)
        return
    ...
    if data.time_list is not None and not np.all(np.in1d(data.time_list, tp.as_timepointarray(data.timepoints.value))):
