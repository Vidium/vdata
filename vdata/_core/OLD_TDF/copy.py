...
def copy_TemporalDataFrame(TDF: Union['dataframe.TemporalDataFrame', 'views.ViewTemporalDataFrame']) \
    ...
    """
    _time_list = TDF.timepoints_column if TDF.timepoints_column_name is None else None

    new_TDF = dataframe.TemporalDataFrame(data=TDF.to_pandas(with_timepoints=True),
                                          time_list=_time_list,
                                          time_col_name=TDF.timepoints_column_name,
                                          timepoints=TDF.timepoints,
