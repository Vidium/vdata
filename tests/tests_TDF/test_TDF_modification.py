...
def test_delete_numerical_column(TDF: TemporalDataFrame):
    if TDF.is_backed:
        TDF.unlock_columns()
    ...
    assert repr(TDF) == f"{prefix}TemporalDataFrame 1\n" \
...
def test_delete_string_column(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
    ...
    assert np.all(TDF.values_num ==
                  np.vstack((np.concatenate((np.arange(50, 100), np.arange(0, 50))),
...
def test_modify_numerical_values_of_existing_column(TDF):
...
def test_create_new_column_numerical(TDF):
...
def test_modify_string_values_of_existing_column(TDF):
...
def test_create_new_column_string(TDF):
...
def test_insert_numerical_column(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
...
def test_insert_string_column(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
...
def test_insert_into_empty_array(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
...
def test_add_numerical_value_to_view(TDF):
    prefix = 'backed ' if TDF.is_backed else ''
    assert repr(TDF + 1) == f"TemporalDataFrame View of {prefix}TemporalDataFrame 1 + 1\n" \
                            "\x1b[4mTime point : 0.0 hours\x1b[0m\n" \
...
def test_add_string_value(TDF):
    prefix = 'Backed ' if TDF.is_backed else ''
    assert repr(TDF + '_1') == f"TemporalDataFrame {prefix}TemporalDataFrame 1 + _1\n" \
                               "\x1b[4mTime point : 0.0 hours\x1b[0m\n" \
                               "   Time-point     col1   col2      col3   col4\n" \
...
def test_add_string_value_to_view(TDF):
    prefix = 'backed ' if TDF.is_backed else ''
    assert repr(TDF + '_1') == f"TemporalDataFrame View of {prefix}TemporalDataFrame 1 + _1\n" \
                               "\x1b[4mTime point : 0.0 hours\x1b[0m\n" \
                               "   Time-point     col1      col4\n" \
                               "50       0.0h  |  50.0  |  350_1\n" \
                               "51       0.0h  |  51.0  |  351_1\n" \
...
def test_subtract_value_to_view(TDF):
    prefix = 'backed ' if TDF.is_backed else ''
    assert repr(TDF - 1) == f"TemporalDataFrame View of {prefix}TemporalDataFrame 1 - 1\n" \
                            "\x1b[4mTime point : 0.0 hours\x1b[0m\n" \
                            "   Time-point     col1    col4\n" \
...
def test_mutliply_by_value(TDF):
    prefix = 'Backed ' if TDF.is_backed else ''
    assert repr(TDF * 2) == f"TemporalDataFrame {prefix}TemporalDataFrame 1 * 2\n" \
                            "\x1b[4mTime point : 0.0 hours\x1b[0m\n" \
                            "   Time-point      col1   col2    col3 col4\n" \
...
def test_mutliply_by_value_to_view(TDF):
    prefix = 'backed ' if TDF.is_backed else ''
    assert repr(TDF * 2) == f"TemporalDataFrame View of {prefix}TemporalDataFrame 1 * 2\n" \
                            "\x1b[4mTime point : 0.0 hours\x1b[0m\n" \
                            "   Time-point      col1    col4\n" \
                            "50       0.0h  |  100.0  |  350\n" \
                            "51       0.0h  |  102.0  |  351\n" \
...
def test_divide_by_value_to_view(TDF):
    prefix = 'backed ' if TDF.is_backed else ''
    assert repr(TDF / 2) == f"TemporalDataFrame View of {prefix}TemporalDataFrame 1 / 2\n" \
                            "\x1b[4mTime point : 0.0 hours\x1b[0m\n" \
                            "   Time-point     col1    col4\n" \
...
def test_add_value_to_empty_array(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
...
def test_add_value_to_empty_array_in_view(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
...
def test_add_str_value_to_empty_array(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
...
def test_add_str_value_to_empty_array_in_view(TDF):
    if TDF.is_backed:
        TDF.unlock_columns()
...
def test_add_TDF_to_TDF(TDF1, TDF2):
    sum_TDF = TDF1 + TDF2
...
def test_subtract_TDF_to_TDF(TDF1, TDF2):
    sub_TDF = TDF1 - TDF2
...
def test_multiply_TDF_with_TDF(TDF1, TDF2):
    mul_TDF = TDF1 * TDF2
...
def test_divide_TDF_with_TDF(TDF1, TDF2):
    div_TDF = TDF1 / TDF2
...
def test_add_to_TDF_with_empty_array_whould_fail(TDF):
    if TDF.is_view:
        del TDF.parent.col1
        del TDF.parent.col2
        TDF = TDF.parent[:]
