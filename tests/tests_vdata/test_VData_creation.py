...
_obs_data = {'data': np.random.randint(0, 20, 6),
...
def test_VData_creation_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=None, obs=None, var=pd.DataFrame(_var_data, index=_var_index_data), 
                    timepoints=pd.DataFrame(_timepoints_data), name='6')
    assert repr(v) == "Empty VData '6' (0 obs x 3 vars over 2 time points).\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_obs_is_df_var_is_None_tp_is_None() -> None:
    v = vdata.VData(data=None, obs=pd.DataFrame(_obs_data, index=obs_index_data),
                    var=None, timepoints=None, name='8')
...
def test_VData_creation_obs_is_df_var_is_None_tp_is_df() -> None:
    v = vdata.VData(data=None, obs=pd.DataFrame(_obs_data, index=obs_index_data), 
                    var=None, timepoints=pd.DataFrame(_timepoints_data),
...
def test_VData_creation_obs_is_df_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=None, 
                    ...
                    name='11')
    assert repr(v) == "Empty VData '11' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_obs_is_TDF_var_is_None_tp_is_None() -> None:
    v = vdata.VData(data=None, 
                    obs=vdata.TemporalDataFrame(_obs_data, 
                                                index=obs_index_data,
                                                time_list=["0h", "0h", "0h", "0h", "5h", "5h"]), 
                    var=None, 
                    timepoints=None, name='12')
...
def test_VData_creation_obs_is_TDF_var_is_None_tp_is_df() -> None:
    v = vdata.VData(data=None, 
                    obs=vdata.TemporalDataFrame(_obs_data, 
                                                ...
                                                time_list=["0h", "0h", "0h", "0h", "5h", "5h"]),
                    var=None, 
                    timepoints=pd.DataFrame(_timepoints_data), name='13')
...
def test_VData_creation_obs_is_TDF_var_is_df_tp_is_None() -> None:
    v = vdata.VData(data=None, 
                    ...
                    timepoints=None, 
                    name='14')
    assert repr(v) == "Empty VData '14' ([4, 2] obs x 3 vars over 2 time points).\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_obs_is_TDF_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=None, 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data), 
                    name='15')
    assert repr(v) == "Empty VData '15' ([4, 2] obs x 3 vars over 2 time points).\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    obs=None, 
                    var=None,
                    timepoints=None,
                    name='17')
...
def test_VData_creation_data_is_df_var_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=None,
                    name='19')
    assert repr(v) == "VData '19' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data_simple), 
                    name='20')
    assert repr(v) == "VData '20' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=None,
                    name='21')
    assert repr(v) == "VData '21' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_id_df_tp_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data_simple), 
                    name='22')
    assert repr(v) == "VData '22' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_is_df_var_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=None, 
                    name='23')
    assert repr(v) == "VData '23' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_is_df_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data),
                    ...
                    timepoints=pd.DataFrame(_timepoints_data_simple), 
                    name='24')
    assert repr(v) == "VData '24' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_is_TDF() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=None, 
                    name='25')
    assert repr(v) == "VData '25' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_is_TDF_tp_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data_simple), 
                    name='26')
    assert repr(v) == "VData '26' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_is_TDF_var_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data), 
                    ...
                    timepoints=None, 
                    name='27')
    assert repr(v) == "VData '27' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_df_obs_is_TDF_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=pd.DataFrame(expr_data_simple, index=obs_index_data, columns=_var_index_data),
                    ...
                    timepoints=pd.DataFrame(_timepoints_data_simple), 
                    name='28')
    assert repr(v) == "VData '28' (6 obs x 3 vars over 1 time point).\n" \
                      "\tlayers: 'data'\n" \
                      "\tobs: 'data', 'data_bis'\n" \
                      "\tvar: 'gene_name'\n" \
                      "\ttimepoints: 'value'"
...
def test_VData_creation_data_is_dict() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    obs=None, 
                    var=None, 
                    timepoints=None, 
                    name='29')
    assert repr(v) == "VData '29' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
...
def test_VData_creation_data_is_dict_tp_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data), 
                    name='30')
    assert repr(v) == "VData '30' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_var_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    timepoints=None,
                    name='31')
    assert repr(v) == "VData '31' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data), 
                    name='32')
    assert repr(v) == "VData '32' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_obs_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    time_list=["0h", "0h", "0h", "0h", "5h", "5h"], 
                    name='33')
    assert repr(v) == "VData '33' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_obs_is_df_tp_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    time_list=["0h", "0h", "0h", "0h", "5h", "5h"], 
                    name='34')
    assert repr(v) == "VData '34' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_obs_is_df_var_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    time_list=["0h", "0h", "0h", "0h", "5h", "5h"], 
                    name='35')
    assert repr(v) == "VData '35' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_obs_is_df_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    time_list=["0h", "0h", "0h", "0h", "5h", "5h"],
                    name='36')
    assert repr(v) == "VData '36' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'", \
...
def test_VData_creation_data_is_dict_obs_is_TDF() -> None:
    v = vdata.VData(data=expr_data_complex,
                    ...
                    timepoints=None,
                    name='37')
    assert repr(v) == "VData '37' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_obs_is_TDF_tp_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data),
                    name='38')
    assert repr(v) == "VData '38' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_obs_is_TDF_var_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    timepoints=None, 
                    name='39')
    assert repr(v) == "VData '39' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_dict_obs_is_TDF_var_is_df_tp_is_df() -> None:
    v = vdata.VData(data=expr_data_complex, 
                    ...
                    timepoints=pd.DataFrame(_timepoints_data),
                    name='40')
    assert repr(v) == "VData '40' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'"
...
def test_VData_creation_data_is_invalid() -> None:
    ...
    assert str(exc_info.value) == "Type '<class 'int'>' is not allowed for 'data' parameter, should be a dict," \
...
def test_VData_creation_with_uns() -> None:
    ...
    assert repr(v) == "VData '46' (6 obs x 3 vars over 1 time point).\n\t" \
                      "layers: 'data'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'\n\t" \
...
def test_VData_creation_full() -> None:
    ...
    assert repr(v) == "VData '47' ([4, 2] obs x 3 vars over 2 time points).\n\t" \
                      "layers: 'spliced', 'unspliced'\n\t" \
                      "obs: 'data', 'data_bis'\n\t" \
                      "var: 'gene_name'\n\t" \
                      "timepoints: 'value'\n\t" \
...
def test_VData_creation_repeating_index() -> None:
    ...
    assert repr(v) == "VData 'No_Name' ([3, 3, 3] obs x 3 vars over 3 time points).\n"\
                      "\tlayers: 'data'\n"\
                      "\tobs: 'col1'\n"\
                      "\ttimepoints: 'value'"
