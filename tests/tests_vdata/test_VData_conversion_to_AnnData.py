...
def v() -> vdata.VData:
    ...
    _RNA = pd.DataFrame(np.vstack((np.zeros((7, 4)), np.ones((3, 4)), 2 * np.ones((10, 4)))))
    _Protein = _RNA * 10

    _RNA["tp"] = _tp
    _Protein["tp"] = _tp

    return vdata.VData({"RNA": _RNA, "Protein": _Protein}, time_col_name="tp")
...
def test_VData_conversion_into_multiple_anndatas(v: vdata.VData) -> None:
    assert (
        repr(v.to_anndata(into_one=False)) == "[AnnData object with n_obs × n_vars = 7 × 4\n"
        "    layers: 'RNA', 'Protein', "
...
def test_VData_conversion_into_one_anndata(v: vdata.VData) -> None:
    assert (
        repr(v.to_anndata(into_one=True)) == "AnnData object with n_obs × n_vars = 20 × 4\n"
        "    obs: 'Time-point'\n"
        "    layers: 'RNA', 'Protein'"
    )
